...
% boulet_parameters and stimulus defined in boulet_stimulation
    ...
    % OUTPUTS:
        % v - voltage response
        % spiketimes
        % vth - threshold over time
        % x_th, x_RS - threshold/noise modifiers (due to ref/sra/fac/acc)
        % x_th/RS_ref/sra/fac/acc - as above but for each effect seperately

%% setup
% loads stimulation type
run boulet_stimulation
% choose stimulation type and enable/disable spatial spread in the file
% neural and simulation parameters are controlled in boulet_parameters script

%% initial conditions
v = vrest * ones(niter,nneurons);
vth = vth0 * ones(niter,nneurons);
x_th = ones(niter,nneurons); % x = x_r * x_sra * (1+y_fac) * (1+y_acc);
    x_th_ref = ones(niter,nneurons);
    x_th_SRA = ones(niter,nneurons);
    y_th_fac = zeros(niter,nneurons);
    y_th_acc = zeros(niter,nneurons);
    y_th_acc_slow = zeros(niter,nneurons);
    z = zeros(niter,nneurons); % time shifted stimulus, used for fac and acc
x_RS = ones(niter,nneurons); % x = x_r * x_sra * (1+y_fac) * (1+y_acc);
    x_RS_ref = ones(niter,nneurons);
    x_RS_SRA = ones(niter,nneurons);
    y_RS_fac = zeros(niter,nneurons);
    y_RS_acc = zeros(niter,nneurons);
    y_RS_acc_slow = zeros(niter,nneurons);

spiketimes = cell(1,nneurons); % array of spike times
spikelast = -10*tmax * ones(1,nneurons);
% sets the time of last spike "long time ago" - prevents errors in code later
firing = zeros(1,nneurons);
tRRP = zeros(1,nneurons);
vnoise = zeros(niter,nneurons);
...
for i = 1:niter-1
        ...
        if niter>10000
            ...
            if i/step == round(i/step)
                disp(strcat(num2str(i/step*10),'% done'))
            end
        end
    
    % linear RC membrane model
    v(i+1,:) = v(i,:) + 1/C .* (Istim(i,:)-(v(i,:)-vrest)./R).*dt; % Euler forwards
    ...
    if noise_mem
        ...
        vth(i,:) = vth(i,:) + vth_sigma(i,:) .* x_RS(i,:) .* x_th(i,:);
        vnoise(i,:) = vth_sigma(i,:) .* x_RS(i,:) .* x_th(i,:);
    end
    ...
    if ref
        ...
        x_th_ref(i+1,tRRP>0) = 1./( 1-exp(-tRRP(tRRP>0)./tau_RRP(tRRP>0)) );
        if noise_ref
            x_RS_ref(i+1,tRRP>0) = 1 + a_RS_ref * exp(-tRRP(tRRP>0)./tau_RRP(tRRP>0));
        end
    end
    ...
    if SRA
        % firing neurons, "jump"
        x_th_SRA(i,firing==1) = x_th_SRA(i,firing==1) + p_th_SRA;
        if noise_SRA
            x_RS_SRA(i,firing==1) = x_RS_SRA(i,firing==1) + p_th_SRA;     
        end
        
        % all neurons, exp decrease towards 1
        x_th_SRA(i+1,:) = x_th_SRA(i,:) + (1 - x_th_SRA(i,:)) .* dt/tau_SRA;
        if noise_SRA
            x_RS_SRA(i+1,:) = x_RS_SRA(i,:) + (1 - x_RS_SRA(i,:)) .* dt/tau_SRA;                
        end                
    end
    ...
    if fac || acc || acc_slow
        ...
        % more than 2 pulses
        y_th_fac(i,Istim(i,:)~=0 & Istim(i+1,:)==0) = 0;
        y_RS_fac(i,Istim(i,:)~=0 & Istim(i+1,:)==0) = 0;
        
        % update FAC
        if fac
            y_th_fac(i+1,:) = y_th_fac(i,:) .* (1-dt/tau_th_fac) + dt * a_th_fac .* z(i,:);
            if noise_fac
                y_RS_fac(i+1,:) = y_RS_fac(i,:) .* (1-dt/tau_RS_fac) + dt * a_RS_fac .* z(i,:);
            end
        end
        
        % update ACC
        if acc
            y_th_acc(i+1,:) = y_th_acc(i,:) .* (1-dt/tau_th_acc) + dt * a_th_acc .* z(i,:);
            if noise_acc
                y_RS_acc(i+1,:) = y_RS_acc(i,:) .* (1-dt/tau_RS_acc) + dt * a_RS_acc .* z(i,:);
            end
        end
        
        % update ACC_slow
        if acc_slow
            y_th_acc_slow(i+1,:) = y_th_acc_slow(i,:) .* (1-dt/tau_th_acc_slow) + dt * a_th_acc_slow .* z(i,:);
            if noise_acc_slow
                y_RS_acc_slow(i+1,:) = y_RS_acc_slow(i,:) .* (1-dt/tau_RS_acc_slow) + dt * a_RS_acc_slow .* z(i,:);
            end
        end
...
%% plot results
v = v(1:niter,:); % trimming away any additional time entries, if any
...
figure(1)
    ...
    axis([0 tmax 0 60]); %1.2*max(Istim(:,1))
