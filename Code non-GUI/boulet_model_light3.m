...
% boulet_parameters and stimulus defined in boulet_stimulation
    % INPUTS: boulet_parameters_light, boulet_stimulation_light
    % OUTPUTS:
        % spiketimes
        
% Light version modifications:
    % sparse version of Istim is used / nonzero values not considered (light3)
    % sparse version of voltage noise is used / noise is defined only for a
        % subset of iterations at each time / see niter_filt variable (light 3)
...
for i = 1:niter-1
    ...
    % calculate current value of stimulus current
    if is_stim_applied(i) == 0
        Istim = 0 * stim_spread;
    else
        counter = counter + 1;
        Istim = Istim_amp(counter) * stim_spread;
    end
    
    % calculate noise subvector
    if mod(i,niter_filt) == 1        
        noise_filt = single(filter(b,a,normrnd(0,1,[niter_filt,nneurons]))); % rand vector, std now smaller!
        vth_sigma = vth0 .* RS_mem/mean(std(noise_filt)) .* noise_filt; % std scaled
        vth_sigma = single(vth_sigma);
        i_filt = mod(i,niter_filt);
    elseif mod(i,niter_filt) == 0
            i_filt = niter_filt;
    end
    
    % linear RC membrane model
    v = v + 1/C .* (Istim-(v-vrest)./R).*dt; % Euler forward
    ...
    if noise_mem
        ...
        vth = vth + vth_sigma(i_filt,:) .* x_th .* x_RS;        
    end
