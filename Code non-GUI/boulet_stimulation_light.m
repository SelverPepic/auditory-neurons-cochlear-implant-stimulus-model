...
% List of stimulation parameters for boulet_model_main_light
    % INPUTS: parameters from boulet_parameters_light
    % OUTPUTS: stimulation parameters (stimulus waveform etc)

% Light version modifications:
    % nonzero values of stimulus are not saved
    % spread function is saved seperately
    % single precision instead of double precision variables are used    

    
%% setup
run boulet_parameters_light
...
%% initialize
is_stim_applied = zeros(niter,1,'uint8');
stim_spread = ones(1,nneurons,'single');

%% stimulus types
% single pulse
if type == 1    
    npulses = 1;
    is_stim_applied(wo:wo+w,:) = 1;
    Istim_amp = A * single(is_stim_applied(is_stim_applied==1));
end
...
if type == 2
    npulses = 2;
    is_stim_applied(wo:wo+w,:) = 1;
    is_stim_applied(wo+wIPI:wo+wIPI + w,:) = 1;
    Istim_amp = A * single(is_stim_applied(is_stim_applied==1));
end
...
if type == 3
    ...
    for k = 0:npulses-1 % sum each pulse separately
        is_stim_applied (wo+k*wIPI : wo+k*wIPI + w,:) = 1;
    end
    
    % modulated pulse train = pulse train * amplitude modulation
    switch mod_type
        case 0
            mod_amp = ones(niter,1,'single');
        case 1
            mod_amp = 1 + mod_depth * sin(2*pi*mod_freq*t'-pi/2);
        case 2
            mod_amp = 1 + mod_depth * sign(sin(2*pi*mod_freq*t'-pi/2));
        case 3
            mod_amp = 1 + mod_depth * sawtooth(2*pi*mod_freq*t');
        case 4
            mod_amp = 1 + mod_depth * sawtooth(2*pi*mod_freq*t',0);
    end
    % AM
    Istim_amp = A * single(mod_amp(is_stim_applied==1));
end

%plot(Istim_amp(:,1)) % plot to check
...
if spread == 1
    ...
    for k=1:nneurons
        stim_spread(k) = exp(-(abs(xneurons(k)-d_stim)/d_spread));
...
%for i = 1:niter
%    if is_stim_applied(i) == 0
%        Istim(i,:) = 0 * stim_spread;
%    else
%        counter = counter + 1;
%        Istim(i,:) = Istim_amp(counter) * stim_spread;
%    end
%end