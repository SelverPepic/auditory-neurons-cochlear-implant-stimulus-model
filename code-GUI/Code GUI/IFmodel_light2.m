...
%% initial conditions
v = p.vrest * ones(1,p.nneurons,'single');
...
x_th = ones(1,p.nneurons,'single'); % x = x_r * x_sra * (1+y_fac) * (1+y_acc);
    ...
    v_temp = zeros(s.w,p.nneurons,'single'); % time shifted stimulus, used for fac and acc
    z = zeros(1,p.nneurons,'single');
...
for i = 1:p.niter-1
    ...
    v = v + 1/p.C .* (s.Istim(i,:) -(v-p.vrest)./p.R).*p.dt;
    % midpoint method
    ...
    if modeltype.fac || modeltype.acc || modeltype.acc_slow
        ...
        v_temp(end,:) = v;
        z = (v_temp(1,:)-p.vrest) ./ (p.vth0_stoch-p.vrest);
        ...
        % update FAC
        if modeltype.fac
            y_th_fac = y_th_fac .* (1-p.dt/p.tau_th_fac) + p.dt * p.a_th_fac .* z;
            if modeltype.noise_fac
                y_RS_fac = y_RS_fac .* (1-p.dt/p.tau_RS_fac) + p.dt * p.a_RS_fac .* z;
            end
        end
        
        % update ACC
        if modeltype.acc
            y_th_acc = y_th_acc .* (1-p.dt/p.tau_th_acc) + p.dt * p.a_th_acc .* z;
            if modeltype.noise_acc
                y_RS_acc = y_RS_acc .* (1-p.dt/p.tau_RS_acc) + p.dt * p.a_RS_acc .* z;
            end
        end
        
        % update ACC_slow
        if modeltype.acc_slow
            y_th_acc_slow = y_th_acc_slow .* (1-p.dt/p.tau_th_acc_slow) + p.dt * p.a_th_acc_slow .* z;
            if modeltype.noise_acc_slow
                y_RS_acc_slow = y_RS_acc_slow .* (1-p.dt/p.tau_RS_acc_slow) + p.dt * p.a_RS_acc_slow .* z;
            end
        end
